...
public class FoodListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(FoodVO vo : list) {
			...
			out.println("<a href=\"FoodDetailBeforeServlet?mode=3&fno="+vo.getFno()+"\"><img src="+vo.getPoster()+" class=img-rounded style=\"width:240px;height:200px\">");
			out.println("</td>");
			out.println("<td width=70%><h3>");
			out.println("<a href=\"FoodDetailBeforeServlet?mode=3&fno="+vo.getFno()+"\">"+vo.getName()+"&nbsp;<span style=\"color:orange\">"+vo.getScore()+"</span>");
			out.println("<h3></td>");
			out.println("</tr>");
			out.println("<tr>");
		...
		if(cookies !=null) {
			for(int i = cookies.length-1;i>=0;i--) { // 최신을 먼저 출력
				if(cookies[i].getName().startsWith("food_")) {
					...
					// 테이블 컬럼에 저장시에는 한개의 값만 저장 -> 관리하기 쉽다.
		...
		if(cList!=null && cList.size()>0) {
			for(FoodVO vo : cList) {
				...
				out.println("<img src=\""+vo.getPoster()+"\" alt=\"Lights\" style=\"width:230px; height:200px\"></a>");
